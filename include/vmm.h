...
// 获取一个地址的页目录项
//取一个地址的高10位，作为页目录中页目录项的偏移地址
#define PGD_INDEX(x) (((x) >> 22) & 0x3FF)

// 获取一个地址的页表项
//取一个地址的中10位，作为页表中页表项的偏移地址
#define PTE_INDEX(x) (((x) >> 12) & 0x3FF)

// 获取一个地址的页內偏移
//取一个地址的低12位，作为4k页内的偏移地址
...
// 一个页目录中页目录项数，同页表，也为1024
#define PGD_SIZE (PAGE_SIZE/sizeof(pte_t))

// 一个页表中的页表项数，一个页表存放在4k的页上，一个页表项4个字节，因此一个页表可以存放1024个页表项
#define PTE_SIZE (PAGE_SIZE/sizeof(uint32_t))

// 映射 512MB 内存所需要的页表数，512M=131072个页=128个页表
