...
{	
	...
	idt_ptr.limits = sizeof(idt_entry_t) * 256 - 1;
...
{
	//interrupt_handlers是一个全局的函数指针数组，所有的中断处理函数指针都在数组中，以中断号为为索引
	//如果中断处理函数存在，则执行此中断处理函数
...
// 设置中断描述符，sel段选择子就是内核代码段描述符，base是代码段中中断处理函数的地址
static void idt_set_gate(uint8_t num, uint32_t base, uint16_t sel, uint8_t flags)
{
